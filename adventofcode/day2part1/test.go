...
func checkRules(line string) bool {
	...
	max, err := strconv.ParseFloat(secondSplit[1], 64)
	if err != nil {
		// built-in function called panic which causes a run time error.
		panic(fmt.Sprintf("Incorrect value for float64 '%val'", max))
	}
	min, err := strconv.ParseFloat(secondSplit[0], 64)
	if err != nil {
		// built-in function called panic which causes a run time error.
		panic(fmt.Sprintf("Incorrect value for float64 '%val'", min))
	}
